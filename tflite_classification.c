...
int main(int argc, char *argv[]) {
    ...
    /* Create the pipeline */
    /*
    pipeline = gst_parse_launch("qtiqmmfsrc name=camsrc ! videoconvert ! videoscale ! video/x-raw,format=RGB,width=1920,height=1080,framerate=30/1 ! \
		                 queue ! videoconvert ! ximagesink", NULL);
    */

    //pipeline = gst_parse_launch("qtiqmmfsrc  name=camsrc ! videoconvert ! videoscale ! video/x-raw,format=RGB,width=1920,height=1080,framerate=30/1 ! jpegenc snapshot=true ! filesink location=img.jpg", NULL);
    
    pipeline = gst_parse_launch("qtiqmmfsrc name=camsrc ! videoconvert ! videoscale ! video/x-raw,format=RGB,width=1920,height=1080,framerate=30/1 ! queue ! videoconvert ! ximagesink", NULL);
    /* Start playing */
